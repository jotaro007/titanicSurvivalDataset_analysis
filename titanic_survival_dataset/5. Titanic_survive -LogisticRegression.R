########################################################################
#Titanic Survival :: Part - 5 : Logistic Regression
########################################################################
########################################################################
...
library(caTools)
set.seed(390)
split = sample.split(train_featured, SplitRatio = .8)
split
...
summary(logit_model2) # AIC : 623.21 :  PClass, Age, SibSp, Parch
...
myy_prediction = predict(logit_model2, test_featured, type = "response")
